...
public class GenericNode<T> implements Node<T> {
    ...
    /**
     * A map that returns the corresponding list of type R
     */
    public <R> Node<R> map(Function<T,R> converter) {
        return new GenericNode(converter.apply(data), next.map(converter));
    }

    public T reduce(T initialValue, BiFunction<T,T,T>combiner) {
        T result = this.data;
        if(!(this.next instanceof EmptyNode)){
            result = (T) next.reduce(result,combiner);
        }
    ...
    public boolean updateIf(Predicate<T> condition, Function<T, T> updater) {
        ...
        if (next != null) {
            updated = next.updateIf(condition, updater) || updated;
        }
        return updated;
    }
    ...
    public Node<T> getNodeAtIndex(int index) {
        if (index == 0) {
            ...
            return null; // 或抛出一个自定义的异常来通知索引越界
        } else {
